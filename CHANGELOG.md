...
- The active runtime browser phase now runs the takeover/fallback contract and a separate finalization-preflight rejection contract before HTTP fixture cleanup.
- The installed PrestaShop 9.1.5 runtime job now contains a dedicated module-store/MariaDB reservation contract before the broader Core process/browser gates; no equivalent 9.2 requirement is added to the current 9.1.5 production milestone.
...
- The atomic reservation-release, cross-customer cart-level reservation, race-safe expiry cleanup, active HTTP/Chromium, finalization-preflight Chromium and new PrestaShop 9.1.5 MariaDB reservation-runtime deltas have not executed in this delta and must not be counted as passing while GitHub Actions quota remains exhausted.
- Representative real payment completion, carrier diversity, true concurrent-tab/customer-binding finalization, zero-total order completion and full account/address browser flows remain release blockers.
...
- The 9.1.5 reservation MariaDB contract uses a synthetic cart/customer identity only in the module-owned reservation table; it never calls `validateOrder()`, creates a Core `Order`, or changes the production readiness gate.
