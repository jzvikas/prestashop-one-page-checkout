...
- The CLI failure diagnostic binds its already loaded fixture `Cart` into `Context::getContext()->cart` before asking Core for `Cart::getOrderTotal()`. This is an in-memory diagnostic bootstrap requirement for Core pricing methods in a no-employee CLI context; it does not mutate the cart or authorize any order path.
...
Executed Native Payment Runtime `34091582104` on commit `07992369931480f44dd36c93e276bb66963602b3` again passed the ambiguity reservation, same-cart TTL recovery and ordinary official `ps_checkpayment` Core-order/cleanup gates. The free-order request again entered Core as `POST /order-confirmation` without a matching router exit before the bounded Chromium timeout. The corrected schema lookup progressed, then the read-only diagnostic itself failed while calculating the cart total because PrestaShop's product-pricing path requires the active cart to be present in the no-employee `Context`. The diagnostic now binds the already loaded fixture cart into `Context::getContext()->cart` before `Cart::getOrderTotal()`, and the smoke contract locks that ordering. No production OPC, payment, reservation or order-creation code changed as part of this diagnostic correction.
