...
5. Before page scripts execute, `page.addInitScript()` registers a listener for the module's real `jzopc:checkout:initialized` lifecycle event.
...
7. The browser then activates only the disposable fixture's shell-service failure marker and reloads `/order`.
8. Failure-mode checkout must prove:
   - no OPC root;
...
9. The marker is removed in nested and outer cleanup boundaries. Reloading the same browser/cart must restore healthy OPC and the same cart ID.
10. This browser contract does not click final submit, send finalization actions, invoke payment handlers or create an order.
11. The standalone browser contract runs before the PHP active-HTTP contract because that later contract owns final product/config teardown.

## Security rationale

The browser is observation and interaction infrastructure, not a new checkout authority. It receives the same server-owned bootstrap already rendered to customers and is prohibited from introducing payment/order shortcuts.
...
This ADR therefore records a configured gate, not passing browser evidence. `INTEGRATION_SHELL_READY` remains `false` until this and the broader identity/address/carrier/payment/concurrency browser matrix actually pass.
