...
`DeliverySectionRenderer` is the third concrete renderer. `PrestaShopCheckoutDeliveryOptionsPresenter` requires the loaded server cart, skips shipping entirely for virtual carts, executes the native `actionCarrierProcess` lifecycle before discovery, and consumes the active Core `CheckoutSession` for `getDeliveryOptions()` / `getSelectedDeliveryOption()`. This deliberately preserves the Core `DeliveryOptionsFinder` or improved-shipment `DeliveryOptionsProvider` selected by `OrderController`, including carrier price/delay presentation and module-specific `displayCarrierExtraContent`. It also preserves `displayBeforeCarrier` and `displayAfterCarrier`.

The delivery template owns only the surrounding namespaced markup and native radio controls. Carrier names, delays, prices and option identifiers are escaped. `displayCarrierExtraContent`, `displayBeforeCarrier` and `displayAfterCarrier` are explicit trusted hook-HTML boundaries and are the only delivery values rendered with Smarty `nofilter`; they originate from PrestaShop's hook/module lifecycle rather than browser input.

The current delivery presenter deliberately fails closed unless the active controller exposes Core `getCheckoutSession()`. That is correct for native `OrderController` rendering, but future module AJAX refresh controllers will need a shared source-backed checkout-session adapter rather than copying carrier discovery or depending on `OrderController`. This boundary must be resolved before a carrier/address mutation endpoint is exposed.

Payment, identity and agreements remain intentionally unregistered until their implementations can preserve the relevant PrestaShop business rules and hook/module output. In particular, payment rendering must preserve `paymentOptions`, `actionPresentPaymentOptions`, payment additional information/forms and any required payment reinitialization lifecycle.

The smoke suite validates renderer delegation and production address/delivery presenters with isolated PrestaShop-shaped fakes, including address ownership filtering, cart/customer mismatch rejection, exact Core delivery-option key preservation, carrier hook ordering and virtual-cart behavior. Full Smarty rendering and service-container wiring still require an actual PrestaShop runtime integration test; CI does not currently boot a PrestaShop installation and this limitation must not be confused with a passing integration test.

## Next application boundary

The next highest-priority milestone is the payment section renderer together with the common checkout-session adapter required by AJAX section refreshes. Payment must use Core payment-option discovery, preserve `actionPresentPaymentOptions`, module forms/actions/inputs/additional information and binary/self-submitting flows, and define the JavaScript reinitialization lifecycle before payment HTML can be safely replaced over AJAX. The shared session adapter must preserve the same Core carrier implementation selected for the current runtime/feature flags so delivery does not diverge between native order rendering and module AJAX controllers.
