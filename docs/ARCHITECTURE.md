...
`AddressesSectionRenderer` is now the second concrete renderer. `PrestaShopCheckoutAddressBookPresenter` reads only the cart-bound `Context` customer, fails closed on a cart/customer identity mismatch, filters each saved address through Core `Customer::customerHasAddress`, loads the Core `Address` object, and uses `AddressFormat::generateAddress()` so displayed lines follow the shop/country address format instead of a module-hardcoded field order. The Smarty template escapes aliases and every generated line, uses native radio/checkbox controls, and exposes separate delivery/invoice groups without depending on theme DOM.

The current address renderer intentionally covers selection of existing saved addresses only. New/edit address forms still belong to the next Phase 2 slice and must reuse PrestaShop address-field/country/state validation rather than duplicate it.

Delivery, payment, identity and agreements remain intentionally unregistered until their implementations can preserve the relevant PrestaShop business rules and hook/module output. In particular, delivery/payment renderers must preserve `actionCarrierProcess`, `displayCarrierExtraContent`, `paymentOptions`, `actionPresentPaymentOptions`, payment additional information/forms and any required payment reinitialization lifecycle.

The smoke suite validates renderer delegation and the production address presenter with isolated PrestaShop-shaped fakes, including ownership filtering and cart/customer mismatch rejection. Full Smarty rendering and service-container wiring still require an actual PrestaShop runtime integration test; CI does not currently boot a PrestaShop installation and this limitation must not be confused with a passing integration test.

## Next application boundary

The next highest-priority milestone is the delivery section renderer, followed by payment. Both are required before a delivery-address mutation can return the complete dependency set safely. The delivery renderer must derive options from PrestaShop's current checkout session and preserve `actionCarrierProcess` / `displayCarrierExtraContent`; the payment renderer must use Core payment-option discovery and preserve payment-module forms/additional information and `actionPresentPaymentOptions`.
